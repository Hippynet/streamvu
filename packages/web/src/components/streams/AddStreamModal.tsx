...
import type { CreateStreamRequest } from '@streamvu/shared'
...
export default function AddStreamModal({ isOpen, onClose, onSubmit }: AddStreamModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Failed to add stream')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="relative transform overflow-hidden rounded-lg border border-gray-700 bg-gray-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-5">
              <h3 className="mb-4 text-lg font-semibold text-white">Add Stream</h3>
